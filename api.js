...
function parseBirthDate(value) { const raw = String(value || '').trim();
...
const top = reduce22(digitSum(birth.year));
const left = reduce22( top + birth.month );
const bridge = reduce22( birth.day + birth.month + top );
const right = reduce22( top + bridge );
const center = reduce22( left + right );
const edge_left = reduce22( top + left );
const edge_right = reduce22( top + right );
...
addRect( 'a', t, 0, w - 2 * t, t );
addRect( 'd', t, h - t, w - 2 * t, t );
addRect( 'g', t, mid - t / 2, w - 2 * t, t );
addRect( 'f', 0, t, t, mid - t );
addRect( 'e', 0, mid, t, mid - t );
addRect( 'b', w - t, t, t, mid - t );
addRect( 'c', w - t, mid, t, mid - t );
return parts.join(''); }
...
export default async function handler( req, res ) { try { if (req.query.mode === 'home') { return res .status(200) .send( 'Numerology Triangle Edges API is running' ); }
const result =
  calculate(req.query.birth_date);

if (
  req.query.mode === 'triangle'
) {
  const svg = `
...
${renderNumber( result.edge_left, 250, 335, '#c62828' )}
${renderNumber( result.edge_right, 550, 335, '#c62828' )}
${renderNumber( result.left, 92, 670 )}
${renderNumber( result.center, 400, 670 )}
${renderNumber( result.right, 708, 670 )} </svg> `.trim();
